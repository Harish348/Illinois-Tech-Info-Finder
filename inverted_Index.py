...
def generate_tf_idf():
    ...
    with open(os.path.join(TF_IDF_FILES_DIR, 'inverted_index.pkl'), 'wb') as f:
...
def generate_tf():
    ...
    :return tf_dict: Term Frequency Dictionary (Mapping of documents with term frequencies of words in it).
    """
...
def generate_idf():
    ...
    # Dictionary to store the IDF of each document.
    idf_dict = dict()

    # Vocabulary of all the words in all the urls.
    all_words_in_all_urls = set()
    ...
    for json_file in os.listdir(DATA_DIR):
        with open(os.path.join(DATA_DIR, json_file), "r") as jf:
            ...
            all_words_in_this_url = set(url_text_dict["WORD_COUNT_MAP"].keys())

            # Append it to main set
            all_words_in_all_urls.update(all_words_in_this_url)

    # IDF OPERATION
    for word in all_words_in_all_urls:
...
def create_directory():
    ...
    if os.path.isdir(TF_IDF_FILES_DIR):
...
def create_vector_space_model():
    """
    Driver function to generate TF-IDF files for further use.
    """
    ...
    if create_directory():
        ...
        # Generate Term Frequency and save pickle.
        tf_dict = generate_tf()
        with open(os.path.join(TF_IDF_FILES_DIR, 'tf.pkl'), 'wb') as fp:
            pickle.dump(tf_dict, fp)

        # Generate Inverse Document Frequency and save pickle.
        idf_dict = generate_idf()
        with open(os.path.join(TF_IDF_FILES_DIR, 'idf.pkl'), 'wb') as fp:
            pickle.dump(idf_dict, fp)

        # Generate TF-IDF and save pickle.
        tf_idf_dict = generate_tf_idf(tf_dict, idf_dict)
        with open(os.path.join(TF_IDF_FILES_DIR, 'tf_idf.pkl'), 'wb') as fp:
